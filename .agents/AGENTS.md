...
- **Headless check:** Use the `compile-check` skill to verify C# code compiles successfully via `dotnet build`.
- **CRITICAL: Refresh Unity:** ALWAYS invoke `refresh_unity` (via the `unityMCP` tool) after making *any* C# script changes. This triggers Unity's AssetDatabase refresh and script compilation in the Editor, ensuring your changes actually take effect for the user.
- **Verify runtime:** Check the Unity Editor logs (via MCP) for runtime errors after refreshing.
